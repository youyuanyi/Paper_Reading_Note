...
<img src=".\img\MoCo\动态字典.png" alt="动态字典" style="zoom:67%;" />
...
<img src=".\img\MoCo\MoCo框架图.png" alt="MoCo框架图" style="zoom:67%;" />
...
#### 为什么MoCo从目标函数和代理任务两个角度去些Related work
...
预训练用了ImageNet-1M，扩展性验证用了Instagram-1b
...
<img src=".\img\MoCo\Ablation momentum.png" alt="Ablation momentum" style="zoom:80%;" />
...
提出把每一个图片看作是一个类别这种代理任务，将代理任务和NCE loss去做对比学习
...
12. 集成模型不能抵抗对抗样本；
13. **模型的线性特性和输入的高维特性是导致模型容易受到攻击的原因**
...
[I-FGSM]: file://E:/研究生学习/论文/研0/Adversarial/1607.02533I-FGSM.pdf
...
Alexey Kurakin, Goodfellow

### Abstract

对抗样本带来了安全隐患，因为即使无法访问潜在的模型，它们也可以用于对机器学习系统进行攻击。到目前为止，所有以前的工作都采用了一种威胁模型，在该模型中，对手可以将数据直接输入到机器学习分类器中。对于在**物理世界中运行的系统**，例如使用摄像头和其他传感器的信号作为输入的系统，并非总是如此。

本文表明了即使在这种现实世界中，机器学习系统也容易受到对抗样本的攻击。我们通过**将从手机相机获得的对抗图像提供给ImageNet Inception分类器并测量系统的分类精度**来证明这一点。我们发现，即使通过相机感知，大部分对抗样本也被错误分类。

### Introduction

介绍了**对抗样本基本概念和它的迁移性**。

所有先前关于神经网络对抗样本的工作都是使用威胁模型，在该模型中，**攻击者可以将输入直接提供给机器学习模型**（数字的，直接传输，没有通过物理世界）。**在进行这项工作之前，还不知道如果对抗样本是在现实世界中构建并通过摄像头观察的，它们是否仍会被误分类**。

许多实际的机器学习系统在物理世界中运行。可能的示例包括但不限于：机器人通过照相机和其他传感器、视频监视系统以及用于图像或声音分类的移动应用程序感知世界。在这种情况下，对手不能依靠输入数据的细粒度每像素修改的能力。因此，出现了以下问题：**是否仍然有可能制作对抗样本例，并对在物理世界中运行并通过各种传感器而非数字表示感知数据的机器学习系统进行对抗攻击？**

为了调查对抗样本在物理世界中的生效情况，我们**使用了预先训练的ImageNet Inception分类器**进行了一项实验。我们**为此模型生成了对抗样本，然后通过手机摄像头将这些示例提供给分类器，并测量了分类准确性**。此方案是一个简单的物理世界系统，该系统通过摄像机感知数据，然后进行图像分类。 我们发现，即使通过相机感知，为原始模型生成的大部分对抗示例仍然分类错误。



### Methods of Genearting Adversarial Images

#### 本文的符号标记

- X：图像，通常为3-D张量（宽度×高度×深度），假设像素的值是[0，255]范围内的整数
- y_true：图像的真实类别
- J(X,y)：给定图像 X 和类 y ，神经网络的交叉熵损失函数。我们有意在损失函数中忽略了网络权重（和其他参数） θ ，因为我们认为在本文中它们是固定的（固定为训练机器学习模型所得到的值）。对于具有softmax输出层的神经网络，给定图像，应用于整数类别标签的交叉熵代价函数等于真实类别的负对数概率：J(x,y)=-log(y|X)
- Clip函数：对对抗图像X'执行逐像素裁剪，使得结果位于原图像X的L∞范围内。<img src=".\img\I-FGSM\Clip函数.png" alt="Clip函数" style="zoom:67%;" />

#### Fast Method（FGSM）

Goodfellow中描述的最简单的生成对抗图像的方法之一是**通过线性化损失函数并求解扰动来最大化受 L∞ 约束的扰动**，可以以解析解的方式完成此操作，但需要进行一次反向传播的调用：

<img src=".\img\I-FGSM\fgsm.png" alt="fgsm" style="zoom:67%;" />

#### Basic Iterative Method

我们引入了一种简单的方法来扩展“快速”方法，我们以较小的步长将其多次应用，并在每一步之后剪切中间结果的像素值，以确保它们位于原始图像的值域中:

<img src=".\img\I-FGSM\基础迭代版本的FGSM.png" alt="基础迭代版本的FGSM" style="zoom: 67%;" />

实验中，α=1，即每次迭代仅将每个像素的值更改了1.选择迭代次数为min(ε+4，1.25ε)

#### Iterative least-likely class method

上述方法都只是试图增加正确类的损失，而没有指定模型应该选择哪个错误类。这样的方法足以应用于诸如MNIST和CIFAR-10之类的数据集，这些数据集的类别数量很少，并且所有类别彼此高度不同。在ImageNet上，具有更多的类别，并且类别之间的差异程度不同，这些方法可能会导致无用的错误分类，例如将一种雪橇犬误用为另一种雪橇犬。

为了产生更多有趣的错误，我们引入了Itertive Least-Likely（最不可能的）类方法。这种迭代方法尝试制作一个对抗图像，将其分类为特定的所需目标类别。对于期望的类别，我们根据图像 X 上训练网络的预测选择了可能性最小的类别：

<img src=".\img\I-FGSM\least like.png" alt="least like" style="zoom:67%;" />



#### Comparison of methods of generating adversarial examples

<img src=".\img\I-FGSM\comparison on methods.png" alt="comparison on methods" style="zoom:67%;" />

在ImageNet的50000张验证集图片上，使用预训练的Inception v3分类器
...
为了研究任意变换对对抗图像的影响，我们引入了**扰动率**的概念。可以将其描述为对抗图像的比例，这些图像在转换后不再被错误分类。

<img src=".\img\I-FGSM\扰动率.png" alt="扰动率" style="zoom:67%;" />

#### Experimental Setup

为了探索物理对抗样本的可能性，我们使用对抗样本的照片进行了一系列实验。我们**打印了干净的图像和对应的对抗样本，拍摄了打印页面的照片，然后从整页的照片中裁剪了打印的图像。**我们可以将其视为黑盒变换，我们称之为“照片变换”。我们计算了照片变换前后的干净图像和对抗图像的准确性，以及经过图像变换的对抗图像的扰动率。

**实验过程**如下：

1. 打印图像。为了减少手工工作量，我们在每张纸上打印了多对干净样本和对抗样本。同样，QR码被放置在打印输出的角落，以利于自动裁剪。所有生成的打印输出图片均以无损PNG格式保存，使用ImageMagick套件中的转换工具以默认设置将一批PNG打印输出转换为多页PDF文件：convert * .png output.pdf。使用Ricoh MP C5503办公打印机打印生成的PDF文件。使用默认的打印机缩放比例，PDF文件的每一页都会自动缩放以适合整张纸。打印机分辨率设置为600dpi；
2. 使用手机相机（Nexus 5x）拍摄打印图像的照片；
3. 自动从照片中裁剪和扭曲验证示例，因此它们将变成与源图像大小相同的正方形。检测照片角落中四个QR码的值和位置。QR码对照片上显示的那批验证示例进行编码。如果任何一个角的检测失败，则将整个照片丢弃，并且不使用该照片中的图像来计算准确性。我们观察到，在任何实验中，不超过所有图像的10％被丢弃，并且丢弃图像的数量通常约为3％至6％。使用透视变换将照片变形以将QR码的位置移动到预定义的坐标中。图像变形后，每个示例都具有已知坐标，可以轻松地从图像中裁剪出来。

对于对抗样本生成方法和 ε 的每种组合，我们进行了两组实验：

- Average case：为了衡量平均案例表现，我们随机选择了102张图像，以给定的 ε和对抗方法在一项实验中使用。此实验估计了在随机选择的照片上成功的频率，随机选择一张图片，而试图将其分类错误。
- Prefiltered case：<img src=".\img\I-FGSM\prefilter_case.png" alt="prefilter_case" style="zoom:67%;" />

#### EXPERIMENTAL RESULTS ON PHOTOS OF ADVERSARIAL IMAGES

总体而言，结果表明，即使经过非平凡的转换：照片变换，某些对抗样本仍会错误分类。这证明了进行物理对抗的可能性。例如，使用的快速方法的对手可能期望大约2/3的图像将被分类为top-1，而大约1/3的图像将被分类为top-5。因此，通过生成足够的对抗图像，与自然输入相比，对手可能会 ε=16 导致更多的错误分类。

### Atrificial Image Transformations

通过打印，照相和裁切图像的过程将这些转换应用于图像，可以将其视为更简单的图像转换的某种组合。因此，为了更好地了解发生了什么，我们进行了一系列实验，以测量人造图像变换的对抗扰动率。我们探索了以下一组变换：**对比度和亮度的变化、高斯模糊、高斯噪声和JPEG编码。**

 对于这组实验，我们使用了从验证集中随机选择的1000张图像的子集。一次选择了1000张图像的子集，因此本节中的所有实验都使用了相同的图像子集。我们针对多对对抗方法和转化进行了实验。对于每个给定的转换和对抗方法对，我们计算了对抗样本，将转换应用于对抗样本，然后根据公式（1）计算了扰动率

从这些实验中可以得出以下一般结论：

1. **快速方法生成的对抗性示例对转换最稳健，而迭代式可能性最小类方法生成的对抗性示例最不稳健**。这与我们的照片转换结果相吻合；
2. top-5的扰动率通常高于top-1的扰动率。可以通过以下事实来解释这一点：为了扰动top-5的对抗样本，转换必须将正确的类别标签推入top-5的预测之一。但是，为了扰动top-1的对抗性示例，我们必须将正确的标签推向“top-1”的预测，这是更严格的要求；
3. 更改亮度和对比度不会对对抗示例产生太大影响。快速和基本的迭代对抗样本的扰动率小于5％，而对于迭代可能性最小的类方法，扰动率小于20％；
4. 与亮度和对比度的变化相比，**模糊、噪声和JPEG编码的扰动率更高**。特别是，迭代方法的扰动率可能达到80％-90％。但是，这些转换都不会破坏100％的对抗示例，这与“照片转换”实验相吻合。

<img src=".\img\I-FGSM\artifical.png" alt="artifical" style="zoom:67%;" />

### Conclusion

在本文中，我们探索了为**物理世界中运行的机器学习系统创建对抗样本**的可能性。我们使用从手机相机拍摄的图像作为Inception v3图像分类神经网络的输入。我们表明，**在这样的设置中，即使通过摄像机输入到分类器中，使用原始网络制作的大部分对抗图像也会被错误分类**，这一发现证明了物理世界中机器学习系统对抗样本的可能性。在未来的工作中，我们希望可以证明除了打印在纸上的图像之外，还可以使用其他种类的物理对象来演示攻击，针对不同类型的机器学习系统（如复杂的强化学习代理）的攻击，在没有访问权限的情况下进行的攻击。模型的参数和体系结构（大概使用了传递属性），以及通过在对抗样本构建过程中显式建模物理尺度转换来实现更高成功率的物理攻击。我们也希望未来的工作将开发出有效的方法来防御此类攻击
...
[DeepFool]: file://E:/研究生学习/论文/研0/Adversarial/1511.04599DeepFool.pdf
...
这些相同的体系结构（state of the art）已显示出对图像的细微的扰动是不稳定的。尽管此现象很重要，但尚未提出有效的方法来准确计算针对此类大规模数据集扰动的、最新分类器的鲁棒性。

本文提出了DeepFool算法，以有效地计算出欺骗深层网络的扰动，从而可靠地量化这些分类器的鲁棒性

### Introduction

<img src=".\img\DeepFool\鲁棒性定义.png" alt="鲁棒性定义" style="zoom:67%;" />

#### 本文贡献

1. 我们提出了一种简单而准确的方法来**计算和比较不同分类器的鲁棒性与对抗性扰动**。
2. 我们进行了广泛的实验比较，结果表明1）我们的方法比现有方法更可靠，更有效地计算对抗性扰动；2）用对抗样本扩充训练数据显着提高了对抗性扰动的鲁棒性。
3. 我们表明，使用不精确的方法来计算对抗性扰动可能会导致关于鲁棒性的不同甚至是误导性的结论

### 二分类中的DeepFool

<img src=".\img\DeepFool\二分类的对抗样本.png" alt="二分类的对抗样本" style="zoom:67%;" />

<img src=".\img\DeepFool\二分类的对抗样本-说明.png" alt="二分类的对抗样本-说明" style="zoom:67%;" />

 改变分类器决策的**最小扰动对应于 x0 在F上的正交投影**。它由解析公式给出：

<img src=".\img\DeepFool\二分类的最小扰动.png" alt="二分类的最小扰动" style="zoom:80%;" />

<img src=".\img\DeepFool\二分类器的计算过程.png" alt="二分类器的计算过程" style="zoom:67%;" />
...
<img src=".\img\DeepFool\多分类器-最小扰动.png" alt="多分类器-最小扰动" style="zoom:80%;" />

<img src=".\img\DeepFool\凸多面体P.png" alt="凸多面体P" style="zoom:67%;" />

<img src=".\img\DeepFool\P的超平面.png" alt="P的超平面" style="zoom:67%;" />

#### General classifier

<img src=".\img\DeepFool\通用的多分类器.png" alt="通用的多分类器" style="zoom:67%;" />



#### Extension to Lp norm

在本文中，我们使用L_2范数衡量了扰动。然而，我们的框架不限于这种选择，并且所提出的算法可以简单地适用于寻找任何 Lp 范数（ p∈[1，∞) ）的最小对抗性扰动。

### Conclusion

 在这项工作中，我们提出了一种DeepFool算法，用于计算欺骗最新分类器的对抗样本。它基于分类器的迭**代线性化，以生成足以更改分类标签的最小扰动**。我们在三个数据集和八个分类器上提供了广泛的实验证据，表明了该方法在计算对抗性扰动方面优于现有方法的优越性以及该方法的效率。由于其对对抗扰动的准确估计，因此所提出的DeepFool算法提供了一种有效且准确的方法来评估分类器的鲁棒性，并通过适当的微调来增强分类器的性能。因此，所提出的方法可以用作可靠的工具，以准确地估计最小扰动向量，并建立更强大的分类器

DeepFool算法的**核心原理**在于**将输入样本沿着最近的分类边界进行微小的调整，使其误分类**。算法的输出是最小扰动向量 δ，用于将输入样本从真实类别 t错误分类到目标类别 。**这个最小扰动向量的范数（通常是L2或L∞范数）可以用来衡量模型的鲁棒性。如果扰动向量的范数较小，说明模型对于输入样本的微小扰动更具鲁棒性。**
...
[DAG]: file://E:/研究生学习/论文/研0/Adversarial/1703.08603DAG.pdf
...
Cihang Xie, Jianyu Wang

### Abstract

将对抗样本应用于目标检测和语义分割领域上。由于检测是一个proposal上的分类，分割是对每个pixel进行分类，因此作者优化了一组pixels/proposals的损失函数，以生成对抗性扰动。

作者提出了一个新的算法（Dense Adversary Generation，DAG），生成大量的对抗性示例。作者发现，基于不同的体系结构甚至针对不同的识别任务，可以在具有不同训练数据的网络之间传递对抗性扰动。此外，总结多种扰动通常会导致更好的迁移性能，这提供了一种有效的黑盒对抗攻击方法。

### Introduction

本文中，定义被扰动的图像为对抗样本，被认为落在**大型高维特征空间的某些区域**上，而这些区域在训练过程中并未进行探索。

基于每个目标都需要经历一个单独的分类的过程，作者提出了DAG。**DAG是一种同时考虑所有目标并优化整体损失函数的算法。它的实现很简单，只需要为每个目标指定一个对抗性标签，并执行迭代梯度反向传播。**

为了提高DAG的鲁棒性，作者更换了IOU，以在优化中保留增加仍然合理的提案数量

我们验证了**当原始图像上的proposals足够密集时，在扰动图像上生成的新proposals上很可能也会产生错误的识别结果**。我们还研究了算法在所考虑提案的密集度方面的有效性和效率

探索了对抗攻击的迁移性。添加两个或多个异构扰动显着提高了可转移性，这提供了一种对某些具有未知结构和/或属性的网络执行黑盒对抗攻击[26]的有效方法
...
$$
定义图像X有N个可识别的目标T={t_1,t_2,...,t_n}\\
对于每个t_n，n=1,2,...,N，都有一个真实标签l_n∈{1,2,...,C}，C是类别个数。\\
PASCAL VOC中是21类（20类+1个背景
$$

##### DAG算法

<img src=".\img\DAG\DAG算法流程图.png" alt="DAG算法流程图" style="zoom:67%;" />

- **X代表图片**

- $$
  \tau={t_1,t_2,...,t_N}代表X的N个target
  $$

- ​	
  $$
  l_n∈{1,2,...,C}代表第n个target的类别，其中C是类别个数，用L={l_1,l_2,...,l_n}表示
  $$

- 

- $$
  f(x,t_n)∈R^c代表X上的第n个target的一个分类分数向量（在softmax之前）。\\
  生成对抗样本的目的是，使得:\\ 
  ∀n,argmax_c{f_c(X+r,t_n)}\neq l_n\\
  其中r就是加入X的扰动
  $$

  

- 为了这个目的，给每个标签加入一个对抗标签l'n。l'n是从其他错误标签里随机采样的。
  $$
  l_n'∈{1,2,...,C}\setminus l_n，用L'={l_1',l_2',...,l_n'}表示\\
  作者用了一个函数Π来实现随机选择排列，则l_n'=Π(l_n)
  $$

- 所有target的损失函数可以写为：

  <img src=".\img\DAG\loss.png" alt="loss" style="zoom:67%;" />

​	目标就是所有目标都被错误预测，这样就能使得L最小化。

- 使用梯度下降来实现最优化。在第m次迭代中，表示当前图像（可能在添加了几个扰动之后）为Xm。把正确预测的叫做active target set:
  $$
  \tau_m=\{t_n|argmax_c\{f_c(X_m,t_n)\}=l_n\}
  $$

- 然后计算所有输入的梯度并累积这些扰动<img src=".\img\DAG\扰动累积.png" alt="扰动累积" style="zoom: 80%;" />

- 为了避免数值不稳定，标准化为:<img src=".\img\DAG\rm标准化.png" alt="rm标准化" style="zoom:80%;" />

  

- 如果所有的目标都被预测为期望结果（即对抗结果），则算法终止 ；或达到最大迭代次数，分割任务设为200，检测任务设为150
- 最后的扰动<img src=".\img\DAG\rm归一化.png" alt="rm归一化" style="zoom:150%;" />

#### Selecting Input Proposals for Detection

DAG算法的一个关键问题是**选择一个合理的τ集作为目标。**

在**语义分割**任务中这个问题容易，**目标是对所有像素产生不正确的分类，因此我们可以将每个像素设置为一个单独的目标**，比如对图像点阵进行密集采样，这样**计算复杂度与像素总数成正比**。

在场景中的**目标检测**中，目标选择就很难了。因为**整个可能的目标（边界盒子的proposal）在数量级上比语义分割大**。用简单点的方法，只考虑通过一个sideway网络产生的proposal。比如：regional proposal 网络（RPN，是fast-RCNN结构中的一部分）。但是发现在对抗扰动r加入到原图像X中时，**可能会因为新的输入X+r产生一个新的proposal，而且网络可能仍然能够正确地对这些新proposal进行分类**。为了解决这一问题，**我们通过增加RPN中NMS（非极大值抑制）的阈值，使proposal变得非常密集。**

在实践中**把IOU从0.7调到0.9，每张图片的proposal从300张上升到3000张**。使用这个更密集的目标集τ，最有可能的目标边界框，至少和一个输入proposal之间的距离只有一个像素，这样可以使得分类错误在相邻边界框之间转移。
...
针对cross-train,cross-network,cross-task三个方面进行了实现，探索DAG产生的扰动的迁移性

发现联合多个扰动的效果比单独施加一个扰动的效果更好，这有助于黑盒攻击

### Conclusion

- 提出了DAG，其基本思想是定义一组密集的目标和一组不同的期望标签，并优化损失函数，以便同时对所有目标产生错误的识别结果

- 发现了DAG可以在不同的任务、模型和训练数据集上具有迁移性

- 发现了联合多个扰动的效果比单个扰动效果更好，这对黑盒攻击有帮助

  

#### 评价

1. 那时候可能还没有I-FGSM，叠加的效果是更好的
2. 这个迁移，网络同构严重，甚至都不能算是异构，在这上面做迁移，感觉说服力不强，文章中所谓的黑盒，在近乎同构的网络上做。。。可能不太好吧
3. 对检测网络的攻击，修改了网络结构，这无法想象，白盒也不能动网络结构呀

## PGD

### 论文地址

[pgd]: file://E:/研究生学习/论文/研0/Adversarial/1706.06083_pgd.pdf
...
[cw]: file://E:/研究生学习/论文/研0/Adversarial/1608.04644_c&w.pdf
...
- **对抗算法分类：**对抗攻击的方式主要分为三大类

- - 基于梯度迭代：FGSM，PGD，MIM，Deepfool
  - 基于GAN ：AdvGAN，AdvGAN++，AdvFacese
  - 基于优化：CW (其实也属于梯度一类）

- **CW 的优势**

- - **CW 攻击产生的对抗样本所加入的扰动，几乎是人眼察觉不出来的**，反观，FGSM和PGD生成的对抗样本所生成的扰动比较糊。
  - **CW的攻击效果更加好**，可以成功攻击很多防御方法，在加有蒸馏防御的分类模型中，CW攻击依然可以高效地攻击成功。
  - 可以自己调节置信度
...
首先明确 CW 攻击是 Target Attack （当然也可以是无目标攻击），**对攻击样例添加难以察觉的扰动，使模型以高置信度给出一个错误的标签**。 该算法**将对抗样本当成一个变量**（后文有解释），那么现在如果要使得攻击成功就要满足两个条件：（1）**对抗样本和对应的干净样本应该差距越小越好**；（2）**对抗样本应该使得模型分类错，且错的那一类的概率越高越好**。这类问题可以表述为一个约束最小化问题：

<img src=".\img\CW\公式1.png" alt="公式1" style="zoom:67%;" />

- <img src=".\img\CW\约束最小化问题.png" alt="约束最小化问题" style="zoom:67%;" />
...
作者用损失函数f6通过**二分查找法**来选择合适的 c进行了实验，将变量c在0.01到100的log空间内均匀地取值，统计攻击成功率及扰动的L2距离，如下图所示：
...
- **PGD**：在攻击的每一个迭代步完成后，都将像素裁剪到[0,1]的范围内，那么算法就会尝试在这个范围内寻找成功攻击的点。但是这样做的缺点是，每一轮迭代完成后，由于裁剪操作的存在，实际上输入下一个迭代步的图像是已经发生了变化的（**它将剪辑后的图片作为下一次迭代的输入，这样每次传入下一步的都不是真实像素值**），已经有研究发现，在优化步骤比较复杂的梯度下降算法（例如加动量的梯度下降）中，这样的裁剪操作会导致很差的效果。
...
[DLR]: file://E:/研究生学习/论文/研0/Adversarial/2003.01690DLR.pdf
...
在过去的几年中，针对对抗攻击的防御策略领域有了显著发展，但是由于**对抗防御的评估通常不够充分**，因此进展受到了阻碍。评估中经常遇到的问题是**攻击的超参数调整不当、梯度混淆或掩蔽**。

在本文中，作者首先提出了由于**步长不理想和目标函数问题**而导致的PGD攻击效果不好的问题。然后，将**新颖的攻击与现有的两个互补攻击相结合**，以**形成无参数、计算可承受且用户独立的攻击组合**，以测试对抗性的鲁棒性。

将组合应用于最近在顶级机器学习和计算机视觉领域发布的论文中的50多种模型。除了其中一种情况，在所有情况下，鲁棒测试准确度均比这些论文中所报道的要低，**通常降低了10％以上**，从而发现了一些防御性的缺陷。

### Introduction

近年来，已经提出了许多防御措施，但是如果采用更强大或更有效的攻击，大多数防御措施都可能被失效。**对抗训练**是迄今为止无法克服的少数几种方法之一。最近的变体正在**利用其他损失**，并通过**生成其他训练数据**或**进行预训练来提高鲁棒性**。另一项工作是**确定性的或基于随机平滑的可证明的防御**。但是，对于具有较大扰动的数据集（如CIFAR-10），这些方法尚不能与对抗训练的经验鲁棒性竞争。

由于防御方法很多都被攻破了，因此该领域目前处于一种状态，即**在没有独立测试的情况下很难判断新防御方法的价值。这限制了该领域的发展**，因为尚不清楚如何区分是不是一个好的防御idea

测试对抗性鲁棒性的最流行方法是PGD攻击，因为它的计算成本低廉，并且在许多情况下性能良好。然而，已经证明，**即使PGD也会失败**，从而导致对鲁棒性的严重高估：**i）固定步长和ii）广泛使用的交叉熵损失**是**潜在失败的两个原因**。作为补救措施，建议**i）一种新的基于梯度的方案Auto-PGD**，它**不需要选择步长**，并且**ii）一种替代损失函数**。这些新颖的工具带来了PGD的两个变体，它们的**唯一可用参数是迭代次数，而其他所有参数均会自动调整**：这是所提出的评估方法的第一部分。

评估不佳的另一个原因是**所使用的攻击之间缺乏多样性**，因为大多数论文仅依赖于PGD或较弱的版本（例如FGSM）给出的结果。例如，两种不同的攻击具有不同的性质：白盒FAB攻击和黑盒Square攻击。重要的是，**这些方法的参数数量有限，可以很好地在分类器和数据集之间进行泛化**。作者**将提出的两个新版本的PGD与FAB和Square Attack相结合，形成了无参数，计算可负担且用户独立的互补攻击组合，以评估对抗性鲁棒性**，称为**AutoAttack**。
...
- 针对PGD进行了修改，提出了APGD
- 将所提出的新的PGD和FAB、Square Attack相结合集成，提出了Auto Attack
- 提出了DLR Loss替代PGD的cross-entropy
...
确定了PGD攻击的标准制定中的三个弱点，以及在对抗性鲁棒性的背景下如何使用它。

- **固定步长是次优的**，即使在处理凸问题（在数学上具有单一全局最小值的问题）时，固定步长也不能保证收敛到最佳解，而且算法的性能很大程度上取决于选择的步长值。
  - 假设有一个简单的线性回归问题，我们希望通过PGD攻击来扰动模型的参数以减小其性能。固定步长意味着在每一次迭代中，攻击者都会按照相同的固定大小改变模型的参数。然而，这个步长可能并不适合所有情况。假设初始的固定步长较大，它可能会导致算法发散而无法收敛。另一方面，如果初始步长过小，算法可能会收敛得非常慢，需要更多的迭代才能达到相同的攻击效果。这表明，固定步长可能无法兼顾不同情况下的最优性能，因此是一个弱点。
- **通常整体攻击方案与攻击预算（可以用于干扰目标模型的最大程度）无关**。攻击算法在进行多次迭代后，可能会使损失函数达到平稳状态，但此时步长可能会变得非常小，从而无法产生更好的攻击效果。因此，基于迭代次数来判断攻击的强度可能会误导人。
  - 考虑一个图像分类任务，我们的目标是通过PGD攻击来扰动图像，使得模型将原本属于“狗”类的图像误分类为“猫”。在攻击预算有限的情况下，我们希望找到最小的扰动，以使分类错误。然而，使用PGD攻击可能会出现以下情况：在几次迭代后，损失函数达到平稳状态，但步长变得非常小，导致攻击不再有效。这可能会误导我们认为攻击已经达到了最佳效果，但实际上，继续迭代可能会产生更大的扰动，从而实现更好的攻击效果。这表明，**根据损失函数的平稳状态来判断攻击的强度可能是不准确的**。
- **算法缺乏趋势判断**。这意味着算法没有考虑优化是否成功发展，并且不能根据情况做出相应的反应。
  - 例如，攻击算法可能会在初始几次迭代中产生大幅度的扰动，但这些扰动可能并没有显著地改变识别结果。攻击算法需要能够判断是否应该继续增加扰动的大小，以使得攻击更加有效。如果算法无法根据优化过程的趋势作出相应调整，那么它可能会在不必要的情况下继续迭代，浪费计算资源。

#### APGD算法

Auto-PGD是一种基于PGD改进， 在其基础上加入动量方法、重启方法及步长衰减， 从而实现自适应地调整迭代步长，避免超参的设置。

其主要思路是将可用的 N_iter 迭代划分为**初始探索阶段**和**开发阶段，在**初始探索阶段，会**在可行集合中搜索较好的初始点**，而在开发阶段，会尝试最大化迄今为止积累的知识。**通过逐步减小步长来管理两个阶段之间的过渡**。

实际上，较大的步长允许在S中快速移动，而**较小的步长更能使目标函数局部最大化**。但是，减小步长不是事先安排的，而是由优化趋势决定的：如果目标的值增长得足够快，则步长很可能是适当的，否则合理地减小步长。

虽然APGD中的更新步骤是标准步骤，但**APGD与常规PGD的区别在于跨迭代选择步长**，**该步长适用于总体预算和优化进度**，并且**一旦步长减小，最大化将从目前找到的最佳点重新开始**。

##### Gradient step

在传统PGD的基础上加了动量， η<sup>k</sup>代表第k次的步长，具体如下：

<img src=".\img\APGD\gradient step.png" alt="gradient step" style="zoom:80%;" />

##### Step size selection

默认 初始步长η(0)=2ε ，定义检查点 **w0=0,w1,...,w_n 代表算法是否要在某个时刻上减半现有的步长**，**减半步长要满足以下2个条件**：<img src=".\img\APGD\步长选择.png" alt="步长选择" style="zoom:67%;" />

##### Restarts from the best point

**如果在检查点 w_j 步长减半**，则将 x<sup>(w_j+1)</sup>=x_max ，即**从目前为止达到最高目标值 f_max 的点重启**。这是有道理的，因为减少 步长 会导致搜索范围更广，并且应该在当前最佳候选解决方案的附近进行。

##### Exploration vs exploitation

<img src=".\img\APGD\探索和开发.png" alt="探索和开发" style="zoom:80%;" />

#### APGD和PGD比较

在获得的CE loss和鲁棒精度方面，将APGD与带有动量的PGD进行了比较，这里重点关注扰动大小为 ε 的 L∞ 攻击，攻击MNIST和CIFAR-10的鲁棒模型（PGD和TRADES）

用动量PGD的1000次迭代，步长为 ε/t ， t∈{0.5,1,2,4,10,25,100} ；APGD，其预算为 N_iter∈{25,50,100,200,400,1000} 次迭代。

在下图中，显示了1000次测试的当前最佳平均交叉熵损失和鲁棒准确度（即攻击无法找到对抗样本的得分百分比）与迭代的关系

<img src=".\img\APGD\APGD和PGD比较.png" alt="APGD和PGD比较" style="zoom:80%;" />

在所有情况下，**APGD的损失都最高**（越高越好，因为这是最大化问题），这对于任何迭代预算都成立。同样，**APGD始终具有最低（更好）的鲁棒性**，因此对这些模型的对抗性较强。可以观察到APGD的自适应行为：**当迭代的预算较大时，目标的值（CE损失）的增长速度较慢，但最终达到较高的值**。这是**由于较长的探索阶段，它牺牲了较小的改进来最终获得更好的结果**。相反，不管步长的选择如何，带有动量的PGD运行趋于稳定在次优值。

### An alternative loss

![交叉熵loss和CW loss](.\img\APGD\交叉熵loss和CW loss.png)

##### Difference of Logits Ratio Loss

Difference of Logits Ratio loss，它既是平移不变性又是缩放不变性，因此与分类器的决策具有相同的自由度。

<img src=".\img\APGD\DLR loss.png" alt="DLR loss" style="zoom:67%;" />

##### APGD和PGD在不同loss上的对比

 在用于优化CE、CW和DLR损失的主要实验中使用的针对 L∞ 鲁棒性训练的所有确定性模型上，将PGD、PGD与动量（与APGD中的动量相同）和具有相同预算的APGD进行了比较。

 对于具有动量的PGD和PGD，使用三个步长大小（ ε/10,ε/4,2ε ， ε 为扰动范数的界）。在43个带有CE的模型，37/43带有CW的和带有DLR的35/43的中，APGD优于6个版本的PGD，其中APGD较差的模型主要是极限步长 2ε 最佳的模型因为防御会导致梯度掩蔽/混淆。

L2 的情况和上面类似。

**两个结论**

- APGD 的性能优于 PGD/带有动量的PGD，无论使用的loss
- DLR loss在 CE loss的基础上有所改善，并且与 CW loss相当，但有更少的失败情况

### AutoAttack: an ensemble of parameter-free attacks

$$
AutoAttack=APGD_{CE}+APGD_{DLR}+FAB+SquareAttack
$$

由于作者希望方法有效，计算可承受且通用，因此选择以下攻击变种来构成自动攻击：**没有随机重启的 APGD_CE ， APGD_DLR** ，即针对Targeted-DLR loss，具有9个目标类别，即FAB的目标版本，即 FAB<sup>T</sup> ，具有9个目标类别，Square Attack具有一次运行5000个查询。**对于每次白盒攻击，使用100次迭代**。尽管运行时取决于模型，模型的鲁棒性甚至是目标网络的框架，但**APGD攻击速度最快，因为每次迭代仅需进行一次正向和反向传递**。AutoAttack的计算预算类似于在评估防御措施时平均使用的预算。

AutoAttack的一个关键特性是其组件的多样性：虽然APGD是针对 Lp 球内任何对抗样本的白盒攻击，但**FAB可以最大限度地减少实现错误分类所必需的扰动范数**，并且尽管它依赖于对数的梯度，但对于受梯度掩盖影响的模型似乎也很有效，如（Croce＆Hein，2020）。另一方面，Square Attack是针对分数有界扰动的基于分数的黑盒攻击，它使用随机搜索并且不利用任何梯度近似。在查询效率和成功率方面，它优于其他黑盒攻击，并且已经证明与白盒攻击相比甚至具有竞争力。两种方法都具有很少能在模型和数据集之间很好地泛化的参数，因此将在所有实验中保持固定不变。集成内的多样性有两个方面的原因：第一，存在一些分类器，某些攻击对其严重失败，但始终至少其中一个能很好地起作用。其次，在同一模型上，各种攻击可能具有相似的鲁棒性，但在不同点上都可以成功：然后，对AutoAttack所做的那样，考虑所有攻击中最坏的情况将提高性能。
...
[ODI-PGD]: file://E:/研究生学习/论文/研0/Adversarial/2003.06878ODI-PGD.pdf
...
[One-Shot]: file://E:/研究生学习/论文/研0/Adversarial/Chen_One-Shot_Adversarial_Attacks_on_Visual_Tracking_With_Dual_Attention_CVPR_2020_paper.pdf
...
Xuesong Chen,Xiyu Yan

@pku,tsinghua

### Abstract

几乎所有计算机视觉中的对抗攻击都针对预先已知的对象类别，这些对象类别可以进行离线训练以产生扰动。但是**对于视觉对象跟踪，跟踪的目标类别通常是事先未知的**。然而，跟踪算法也存在被攻击的潜在风险，可能被恶意用来欺骗监控系统。同时，**由于跟踪目标可以是任意类别**（free-model），因此对抗攻击跟踪仍然是一项具有挑战性的任务。因此，为了让更多人关注潜在风险，作者研究了对跟踪算法的对抗攻击。

[^free-model]: 本文中作者提到一个概念，就是之前的对抗攻击都是研究已知类别的，即**类别是预先设定且已知的**。但对于视觉追踪来说，**追踪的对象可以是任意类别**，并不能预先知晓，这种任意类别的模型在文中被描述为free-model。

作者提出了一种**one-shot adversarial attack**，**用于给free-model产生单目标跟踪的对抗样本**，其中仅在初始帧中对目标patch添加轻微扰动会导致最先进的跟踪器丢失后续帧中的目标。具体来说，所提出的攻击的优化目标由两个部分组成，并利用了双重注意机制

- 第一个组件通过**使用置信度注意力优化批量置信度损失**采用有target attack策略
- 第二个组件通过**使用通道注意力优化特征损失**来应用一般扰动策略

### Introduction

#### 难点

**攻击跟踪器以丢失连续视频帧中的对象是一项具有挑战性的任务。**

- 首先，**在线视觉跟踪**由于free-model跟踪目标和连续视频帧，**无法预先知道被跟踪的类别并提前学习**。
- 其次，**很难设置优化目标来生成对抗样本**，因为对跟踪任务的成功攻击与在多分类任务上的攻击明显不同，后者只能最大化具有第二高置信度的类的概率。具体来说，**每一帧中的跟踪任务与将所有候选框分类为一个正样本，其他为负样本的任务相同。这种特殊的二分类问题如果只选择一个候选框来增加其置信度，就很难进行成功的攻击。**
- 最后，与检测中使用的NMS算法不同，**高斯窗**被广泛应用于修正跟踪任务中的框置信度，**这导致难以平衡攻击的强度和成功率**。例如，当只考虑攻击效果时，离物体最远的bbox就是最好的扰动目标。然而，远处框的置信度更受高斯窗的抑制，选择这些框作为目标可能会导致攻击失败。

为了应对这些挑战，在本文中，作者研究了针对视觉对象跟踪的对抗攻击。攻击目标是一系列**基于Siamese网络的跟踪器**，其中由于离线学习的独特优势和放弃相似性更新，跟踪精度和效率得到了很好的平衡。对于这些跟踪器，我们提出了一种**one-shot攻击框架——仅轻微扰动视频初始帧中目标块的像素值，从而达到在后续帧中攻击跟踪器的目标，即无法跟踪SiamRPN。**



#### 优化目标

作者提出的攻击方法的**优化目标**由**两个Loss和其对应的注意力技术**组成。

- 作者将基于Siamese network的跟踪任务视为一个特殊的分类任务，即跟踪的候选框被视为分类标签，以最大置信度成功匹配目标模板和候选框。因此，我们可以有针对性地干扰跟踪器，使其匹配“最佳box”。在这里，**我们通过抑制优秀candidates的confidence并激励中等candidates的confidence来优化批量confidence loss**。为了进一步区分高质量的候选框，**采用面向距离的注意力机制来拉大优秀候选框之间的距离**
- 我们通过**优化特征损失**来应用一般的扰动策略，**最大化特征空间中干净图像与其对抗样本之间的距离，以进行强大的攻击。**为了提高攻击的泛化性，在有限的扰动条件下，考虑到了**以激活为导向的特征图的特征信道注意力（the feature channel-wise activation-oriented attention of feature maps）**



### Related work

介绍了Adversaria Attack和Siamese Network-based Tracking
...
重新讲了下Siamese Network是怎么做VOT的，然后讲了对VOT进行对抗攻击的难点，并提出了作者的解决方法

- 首先，由于类别未知，有必要**为任意跟踪目标生成匹配的对抗性扰**动。因此，作者提出**只在视频的第一帧添加perturbation**
- 第二，作者提出通过**优化一个batch confidence loss来学习attack perturbations**，目标是减少high-quality boxes的confidence，增加low-quality boxes的confidence，从而尽可能使tracker犯错。此外，提出了设计一个**feature loss**来学习使得攻击有好的泛化性。
- 最后，为了提高攻击效果，作者**给两个loss都添加了attention技术**。在batch confidence loss中，使用置信度注意力来明显抑制不同的候选者；在**feature loss**中，**对特征图的通道添加注意力，通过注意力特征来区分不同通道的重要性**

#### One-shot Attack with Dual Loss

给定初始帧和target的Ground-truth box（GT Box)，我们可以得到target patch `z`，我们的目标就是one-shot attack可以产生只在初始帧上的扰动`Δz`，生成扰动图像`z*=(z+Δz)`使得tracking结果远离GT Box，因此对于`z*`的优化目标为：

<img src=".\img\One-shot\z的优化.png" alt="z的优化" style="zoom:67%;" />

##### Batch Confidence Loss

对于搜索区域X，它在target周围并且是target的两倍size，包括**n个candidates {`x1,...,xn`}**

定义`f(z,xi)`为tracking model的输出，`z`是input frame，`x_i`是第i个candidate的confidence。

对于n个candidates，根据大小进行排名得到`R_{1:n}`
...
为了提高攻击效果，作者为loss引入了dual attention

##### Confidence Attention

通过将confidence attention机制应用到损失函数中，我们可以区分对于不同confidence的candidates的抑制和刺激程度。

<img src=".\img\One-shot\confidence attention.png" alt="confidence attention" style="zoom:67%;" />

- `d(xi)`代表了在有序的confidence list中，第i个candidate `xi`和第一个`x1`之间的坐标距离
- a,b,c都是控制超参数
- c代表收缩率
- a和b限制了权重`wi`的范围
...
由于扰动条件有限，所以使用了channel-wise activation-guided attention来区分不同channels的重要性，以此保证one-shot攻击的泛化性。

<img src=".\img\One-shot\feature attention.png" alt="feature attention" style="zoom:67%;" />

##### Dual Attention Loss

结合以上两个loss，得到dual attention loss，**目标是最小化整个L**

<img src=".\img\One-shot\dual attention loss.png" alt="dual attention loss" style="zoom:80%;" />

##### 伪代码

<img src=".\img\One-shot\One-shot算法伪代码.png" alt="One-shot算法伪代码" style="zoom:80%;" />



### Attack Evaluation

在OTB100，LaSOT和GOT10K 三个数据集上进行了实验。

实验用了4个模型：SiamFC，SiamRPN，SiamRPN++和SiamMask，其中SiamRPN++用了两种不同的back bone（ResNet-50和MobileNet-v2)。



### 个人评价

应该是一项在VOT领域做Adversarial attack的工作，提出了两种Loss和attention结合的one-shot attack方法

但是缺少迁移性验证（作者只在Siam-base的模型上做了实验），我觉得不够有说服力。
...
[Adversarial Patch]: file://E:/研究生学习/论文/研0/Adversarial/1712.09665Adversarial_Patch.pdf
...
提出了一种在现实世界中创建通用的、健壮的、有针对性地image patches的方法。

通用性是因为这些patches可以被用来攻击任何场景

健壮性是因为它们可以在各种各样的转换下工作

有针对性是因为它们可以使分类器输出任何目标类

这些images patches可以被打印下来添加到任何场景或者拍照给classifiers。

### Introduction

对抗样本通常会**对每个像素进行少量修改**，可以**使用多种优化策略找到它们**，例如L-BFGS，快速梯度符号方法（FGSM），DeepFool，投影梯度下降（PGD）以及最近提出了离散化输入的Logit空间投影梯度上升（LS-PGA）

讲了对抗样本在真实世界中的研究工作：打印、3D打印、对抗眼睛等

大多数先前的工作都集中于攻击和防御输入的细微的变化。我们构建的攻击不会试图将现有目标巧妙地转换为另一项。取而代之的是，**这种攻击会产生与图像无关的补丁**，这对神经网络极为重要。然后**可以将该补丁放置在分类器view内的任何位置**，**并使分类器输出目标类**。由于**此补丁与场景无关**，因此，攻击者无需事先了解照明条件、摄像机角度、被攻击的分类器类型甚至是场景中的其他目标，即可发起物理世界的攻击。

### Approach

#### 传统的查找target对抗样本的方法

<img src=".\img\Adversarial_Attack\传统方法.png" alt="传统方法" style="zoom:80%;" />

#### 作者的方法

 相反，我们通过用patches完全替换图像的一部分来发起攻击。

通过mask patch以使其具有任何形状，然后训练各种图像，对每个图像中的补丁应用随机平移、缩放和旋转，并使用梯度下降进行优化。
...
<img src=".\img\Adversarial_Attack\EOT.png" alt="EOT" style="zoom:67%;" />

- `X`：训练集的图像
- `T`：patch上的transformations的分布
- `L`：图像位置的分布

这种期望是基于图像的，这鼓励训练有素的补丁工作，而不管背景是什么。这与大多数先前关于对抗性扰动的工作不同，因为这种扰动是通用的，因为它适用于任何背景。

我们还考虑了**伪补丁**，这些补丁被迫看起来像给定的起始图像。在这里，我们**仅向补丁目标添加 ||p−p_orig||∞<ε** 形式的约束，这将迫使最终补丁在某个初始补丁程序的 L∞ 范数的 ε 内。

我们认为，这种攻击利用了构建图像分类任务的方式。尽管图像可能包含多个目标，但是只有一个目标标签被认为是真实的，因此**网络必须学会检测帧中最“突出”的目标**。对抗补丁通过产生比真实世界中的对象显着得多的输入来利用此功能。因此，当攻击目标检测或图像分割模型时，我们期望将目标烤面包机补丁分类为烤面包机，并且不会影响图像的其他部分。

### Experimental Results

为了测试我们的攻击，我们比较了2种白盒攻击、1种黑盒攻击和control patch的功效。白盒整体攻击联合训练了五个ImageNet模型的单个补丁：inceptionv3，resnet50，xception，VGG16和VGG19。然后，我们**通过平均所有五个模型的获胜率来评估攻击**。白盒单一模型攻击的功能相同，但仅针对单一模型进行训练和评估。**黑盒攻击联合训练了四个ImageNet模型中的单个补丁**，**然后评估了第五个模型上的黑盒攻击**，**我们在训练过程中没有对此模型进行访问**。控件是烤面包机的图片。在训练和评估期间，补丁会重新缩放，然后以数字方式插入到随机ImageNet图像上的随机位置，如之前示意图所示。

可靠地欺骗模型所需的补丁大小明显大于对单个图像执行非目标攻击所需的补丁大小以及白盒设置中的单个位置

### Conclusion

 我们证明了我们**可以生成一个通用的、健壮的、有针对性的补丁**，**无论补丁的规模或位置如何，它都会欺骗分类器，并且不需要了解攻击场景中的其他目标**。我们的攻击**在现实世界中可行，并且可以伪装成无害的标签**。这些结果表明，可以离线创建然后广泛共享的攻击。

 在防止对自然图像的小 Lp 扰动方面进行了大量工作，至少部分是出于安全方面的考虑。 这项工作的部分动机是，潜在的恶意攻击者可能不会担心对自然图像产生小的或无法察觉的干扰，而是可能会选择对输入进行更大、更有效但更明显的干扰-尤其是在设计模型时，抵抗小的 Lp 扰动。
...
[DPATCH]: file://E:/研究生学习/论文/研0/Adversarial/1806.02299DPATCH.pdf
...
提出了DPATCH：一种针对主流目标检测器（Faster R-CNN和YOLO）的基于黑盒对抗patch的攻击。

与仅使用图像级分类器的original adversarial patch不同，我们的**DPATCH同时攻击bouding box回归和对象分类**，从而使其预测失效。

与以前的工作相比，DPATCH具有以下几项吸引人的特性：（1）DPATCH可以**执行无针对性和有针对性**的有效攻击，**将Faster R-CNN和YOLO的mAP分别从75.10％和65.7％降低到1％以下**；（2）DPATCH的**体积小，其攻击效果与位置无关，因此实施实际攻击非常实用**；（3）DPATCH展示了在不同检测器以及训练数据集之间的巨大**可移植性**。

### Introduction

#### 问题

许多现有的对抗攻击都专注于学习整张图像的加性噪声，其中通过将较小的扰动加入到输入样本中来控制深度学习系统的预测。尽管加入的噪声很小，有时甚至是人眼看不见的，但是这些方法需要操纵整个图像，并且在现实世界的物理攻击中变得不切实际。

介绍了Adversarial Patch：

Adversarial Patch会误导CNN分类器，**以预测与粘贴的补丁耦合到目标类的任何对象**，而**不管对象的比例、位置和方向如何**。这种对抗性攻击的有效性也**已经在黑盒设置中得到了证明**，在这种设置中，攻击者不知道被攻击神经网络的结构和参数。

但是这种Adversarial Patch无法欺骗目标检测器，如图所示：

<img src=".\img\DPATCH\原始方法攻击失败.png" alt="原始方法攻击失败" style="zoom:67%;" />

原因在于**检测器体系结构**：现代目标检测器**首先将具有不同大小的物体定位在图像上的不同位置**，**然后进行分类**。因此，**在这种情况下需要攻击的目标数量要比纯分类应用程序中的数量大得多**。例如，F**aster R-CNN会生成大约20k的regions域用于分类，这远远超出了原始对抗补丁可以有效攻击的范围**。

#### 作者提出的改进

我们的主要观点在于，**边界框回归和对象分类都需要同时受到攻击**。基于此观察，我们提出**DPATCH：一种经过迭代训练的对抗补丁，可以有效地攻击主流目标检测器**。较小的DPATCH（例如40 x 40像素区域）可以执行无目标攻击和有目标攻击：**在无目标攻击中，对象检测器无法找到包含正常对象的正确区域**；**在有针对性的攻击中，对象检测器只能检测DPATCH，而忽略图像上的任何其他对象。**



### Related Work

**攻击深度学习系统**。CNN已被证明容易受到对抗攻击，其中CNN的分类结果可以通过在输入示例中添加小扰动来进行操纵。可以通过基于梯度的算法（例如快速梯度符号方法（FGSM）和投影梯度下降（PGD））或迭代方法（例如DeepFool和Carlini-Wagner（CW）攻击）来制作对抗样本。基于对抗性攻击的概念，设计了“对抗性眼镜”来欺骗人脸识别系统，以便该系统将佩戴者识别为其他人。**先前有关对抗性攻击的大多数研究都是关于像素级加性噪声的。但是，这些攻击会少量改变输入图像的所有像素，这在现实世界的视觉系统（如网络摄像头和自动驾驶汽车）中往往不可行。**

**对抗patch**。为了**对现实世界的视觉系统进行通用攻击**，Google最近设计了一种通用的、强大的对抗性patch程序，可以将其应用于实际的物理场景中，从而使分类器输出任何目标类。**基于目标检测网络用于检测图像中最“显着”目标的原理，可以将对抗性patch训练为比场景中其他对象更“显着”。**具体而言，**对patch进行训练以优化目标类别的预期概率。在训练过程中，将patch以随机比例和旋转的随机位置应用于场景。该过程使patch能够抵抗移位、缩放和旋转。最终，将场景中的对象检测为补丁的目标类别。**
...
Faster R-CNN是典型的two-stage检测器。第一阶段是通过深度的全卷积网络提出候选区域，而第二阶段是使用候选区域的Fast R-CNN检测器.

**为了攻击像Faster R-CNN这样的two-stage检测器，有必要使正确区域候选的生成无效。我们的目的是使DPATCH所在的区域成为唯一有效的RoI，而其他可能的候选应被视为不拥有对象，因此应被忽略**。

##### YOLO

**攻击YOLO时，应将DPATCH所在的网格视为对象，而其他网格则应忽略。也就是说，包含DPATCH的网格比其他具有普通对象的网格具有更高的置信度得分**

#### DPATCH Formulation

##### 原始的Adversarial Patch

Google提出的Adversarial Patch的目标是当patch应用时，最大化CNN分类器的loss。

<img src=".\img\DPATCH\Adversarial Patch.png" alt="Adversarial Patch" style="zoom:80%;" />

##### 目标检测的Adversarial Patch

<img src=".\img\DPATCH\DPATCH的优化目标.png" alt="DPATCH的优化目标" style="zoom:80%;" />

- 对于untarget attack，目标是找到一个patch `Pu`，能**最大化**真实标签`y_hat`和候选框`B_hat`的loss
- 对于target attack，目标是找到一个patch `Pt`，能**最小化**目标类标签`y_t`和边界框标签`B_t`的loss



#### 检测器攻击系统

<img src=".\img\DPATCH\DPATCH框架图.png" alt="DPATCH框架图" style="zoom:80%;" />

- 在输入图像进入检测器系统之前将一个随机初始化的DPATCH模式添加到输入图像中，并将**GT**定义为**[DPATCH x，DPATCH y，DPATCH w，DPATCH h，target label]**
- DPATCH x和DPATCH y表示DPATCH的位置，而DPATCH w和DPATCH h表示宽度和高度。Bouding box标签为[DPATCH x，DPATCH y，DPATCH w，DPATCH h]，类似于检测器边界框的基本原理。
- 对于无目标攻击，target label定义为0。对于目标攻击，target label等于目标类别的标签

##### DPATCH的设计

DPATCH默认是一个 40×40 的正方形，它附着在图像的左上角。

1. **随机放置的DPATCH**：为了分析不同位置的影响并使DPATCH移位不变，我们**在保持DPATCH中所有像素相同的同时随机移位DPATCH的位置**。具体来说，在每次训练迭代时准备DPATCH时，我们会**随机初始化shift的值，但其中的像素点保持不变**。在这种情况下，数据集中的**每个图像都将附有相同的DPATCH，但位置不同**。随机放置的DPATCH的大小仍为 40×40 ，与默认设置相同；
2. **不同目标类别的DPATCH**：由于用于训练检测器的数据集中有10种以上的物体类别，例如Pascal VOC 2007中有20种类别，因此如果设置了mAP，mAP是否会下降到相似的值就很有趣。具有不同标签的DPATCH。 例如，对于有针对性的攻击，我们可以将GT设置为[0，0，40，40，15]，**15是代表人的标签索引。也就是说，我们希望检测器只能识别DPATCH并将DPATCH归类为人**。在我们的实验中，我们从Pascal VOC 2007中随机选择了四个类，分别是自行车、船、牛和电视，并评估它们的攻击效果；
3. **不同大小的DPATCH**：DPATCH大小是另一个可能影响DPATCH攻击有效性的预定因素。在难以检测和防御的较小补丁之间存在折衷，而较大的补丁提供更好的攻击效果。在我们的实验中，我们产生了三种不同大小的DPATCH，分别是20×20、40×40和80×80，以测试它们的攻击效率。通过这种方式，我们可以更好地了解DPATCH大小与其攻击效果之间的关系，从而可以找到有意义攻击的补丁的最小可能大小。（1）DPATCH可以对主流的现代探测器架构（例如two-stage探测器Faster R-CNN和one-stage检测器YOLO）进行有效的攻击；（2）DPATCH的体积小，其攻击效果与位置无关，因此实施实际攻击非常实用；（3）DPATCH展示了不同检测器体系结构和训练数据集之间的巨大可移植性。例如，在Faster R-CNN上训练的DPATCH可以有效地攻击YOLO，反之亦然。我们的实验表明，即使在不了解被攻击网络的体系结构和参数的情况下，DPATCH仍可以在黑盒设置下执行有效的攻击。DPATCH的成功实现还说明了现代检测器体系结构对于此类基于补丁的对抗攻击的固有脆弱性。

#### DPATCH的可迁移性

通常不知道检测器由哪种体系结构训练。在这种情况下，由YOLO训练的DPATCH也可以欺骗Faster R-CNN，或者由Faster R-CNN训练的具有不同分类器的DPATCH可以相互欺骗是更有意义的。为了验证这个想法，我们通过YOLO训练了DPATCH，然后将训练后的DPATCH添加到输入图像，让Faster R-CNN作为检测器。如果Faster R-CNN无法正确检测输入图像中的物体，则我们的DPATCH攻击应被视为可在检测器之间转移。此外，我们在COCO上训练了DPATCH，然后攻击了受Pascal VOC训练的检测器。如果mAP明显降低，则应将DPATCH视为可在数据集之间转移。

### Experiments

很夸张就对了，很多实验结果mAP都下降到了0

### Conclusion

- DPATCH可以对主流的现代检测器架构（例如two-stage探测器Faster R-CNN和one-stage检测器YOLO）进行有效的攻击
- DPATCH的**体积小，其攻击效果与位置无关，因此实施实际攻击非常实用**

- DPATCH展示了不同检测器体系结构和训练数据集之间的巨大可移植性。例如，在Faster R-CNN上训练的DPATCH可以有效地攻击YOLO，反之亦然。我们的实验表明，即使在不了解被攻击网络的体系结构和参数的情况下，**DPATCH仍可以在黑盒设置下执行有效的攻击**。DPATCH的成功实现还说明了现代检测器体系结构对于此类基于补丁的对抗攻击的固有脆弱性。
...
[ILA]: file://E:/研究生学习/论文/研0/Adversarial/1911.13038Indirect_Local_Attacks.pdf
...
### Abstract
 ...
 在本文中，我们表明，生成的网络不仅对扰动影响整个输入图像的全局对抗攻击敏感，而且**对间接局部攻击敏感，**在**间接局部攻击**中，**扰动被限制在一个不与图像重叠的小图像区域**而可以欺骗网络

为此，我们引入了一种**间接攻击策略**(indirect attack strategy)，即**自适应局部攻击(adaptive local attack)**，旨**在找到最佳图像位置进行扰动，同时保留该位置的标签并生成逼真的分割图**。此外，我们提出了**在全局图像级别上以及为获得欺骗区域的像素方向定位的攻击检测技术**。

结果发现**更精确的语义分割网络对间接本地攻击更加敏感**

### Introduction

语义分割的模型为了利用上下文信息，引入了空洞卷积、池化操作、注意力机制等，但是这种利用上下文信息的模型对间接局部攻击很敏感。

作者研究了**间接局部攻击，其扰动会对象外部的区域进行干扰**。

#### 间接局部攻击的基本思想

- **扰动可能发生在图像中除目标对象之外的任何位置**
- 然后，我们切换到**更实际的定位间接攻击案例**，并设计一种基于群体稀疏性的策略，**将受干扰的区域限制在目标对象之外的一小部分区域**
- 为了使我们的攻击保持现实和不可察觉，我们在没有标准的动态对象信息的情况下以标准约束设置执行攻击。

<img src=".\img\Indirect local attack\攻击效果.png" alt="攻击效果" style="zoom:80%;" />

实验发现：**更准确的语义分割网络对间接局部攻击更为敏感**，作者是认为**因为使用上下文可以提高分割的准确性，还可以增加网络的感受范围，从而使扰动传播到更远的图像区域**。

由于分割网络对间接本地攻击的这种不稳定的敏感性，因此，我们将重点转向对抗性攻击检测。与仅有的两篇针对语义分割的攻击检测技术有所不同，我们不仅在全局图像级别，而且**在像素级别局部执行检测**。具体来说，我们**引入一种方法来定位其预测受攻击影响的区域，而不是受干扰的图像区域**。在自动驾驶场景中，这将使人们可以更直接地将注意力集中在潜在危险本身上，而不是将注意力集中在造成这些危险的图像区域上。

总而言之，我们的贡献如下。

1. 引入了针对语义分割网络的间接局部对抗攻击的思想，该思想更好地反映了潜在的现实世界中的危险。
2. 设计了一种自适应的、依赖于图像的局部攻击策略，以找到在静态图像区域中受到干扰的最小位置。
3. 展示了现代网络对于通用的、与图像无关的对抗补丁的脆弱性。
4. 研究了上下文对网络对我们的间接局部攻击的敏感性的影响。
5. 介绍了一种在图像级别和像素级别检测间接局部攻击的方法。
...
但是，**这些工作都没有对攻击在输入图像中的位置施加任何约束**。这样，整个图像都受到了干扰，尽管当攻击者可以访问图像本身时有效，但**不会允许人们实际修改场景以愚弄例如自动驾驶汽车**。从本质上讲，文献[9]显示了通用的目标patch将识别系统欺骗到特定的目标类别。后来，在更现实的环境中研究了patch攻击，结果表明，在交通标志上放置精心设计的小补丁可以欺骗分类网络做出错误的决定。**尽管这些工作集中在分类上，但补丁攻击已扩展到对象检测和光流**。我们的工作在以下方面与这些方法有根本的不同。首先，**这些方法均未优化补丁扰动的位置**。其次，一些工作为每个目标类别学习一个单独的扰动，在测试时，它使攻击者仅将预测更改为一个类别。虽然这适合识别，但不适用于我们的分割设置，**在分割设置中，我们试图将动态对象错误分类为不同的背景类，以生成逼真的分割图**。第三，与一些工作不同，**我们的扰动是不可察觉的**。最后，虽然某些工作中的扰动涵盖了应该被错误分类的区域，并且扰动影响了被扰动区域中的预测，但**我们的目标是设计一种仅影响被扰动区域以外目标区域的攻击区域**。

换句话说，我们研究了扰动在目标区域之外的间接局部攻击的影响。此外，我们与其他基于patch的攻击的不同之处在于，我们研究了针对语义分割的定位攻击，以了解不同网络所利用的上下文信息的影响，并在图像和像素级别引入了检测策略。在大多数现有文献中，我们将重点放在白盒设置上有以下三个原因：

1. 发展目前缺乏的有效的语义分割防御机制，需要评估语义分割网络对最强攻击（即白盒攻击）的敏感性；
2. 最近的模型提取方法使攻击者可以获得部署模型的近似值；
3. 虽然分类有效，但观察到黑盒攻击在语义分割体系结构之间的传递性很差，尤其是在目标场景中。

### Indirect Local Segmentation Attacks

语义分割的优化目标一般为最小化以下loss function

<img src=".\img\Indirect local attack\语义分割的loss.png" alt="语义分割的loss" style="zoom:67%;" />

**用F∈{0,1}<sup>W X H</sup>表示fooling mask。如果第 j 个像素位置被攻击者误分类，则 Fj = 1，并且 Fj = 0 是应保留的预测标签**

#### Indirect Local Attacks

##### 非目标攻击

<img src=".\img\Indirect local attack\非目标攻击.png" alt="非目标攻击" style="zoom:80%;" />

目标是最小化targeted regions的y<sup>pred</sup>的可能性，最大化其他区域的可能性。

##### 目标攻击

<img src=".\img\Indirect local attack\目标攻击.png" alt="目标攻击" style="zoom:80%;" />

以上两种方法都用PGD进行迭代训练

了进行间接局部攻击，我们只需定义掩码M和F，使它们不相交，即M⊙F = 0，其中⊙是逐元素乘积。

#### Adaptive Attacks

实际上，人们可能希望找到进行攻击的**最佳位置**，同时**使攻击尽可能地局部化**，在本节中介绍一种**通过在扰动掩码上执行结构化稀疏性**来实现此目的的方法。

<img src=".\img\Indirect local attack\自适应.png" alt="自适应" style="zoom:67%;" />

#### Universal Local Attacks

尽管adversarial patch针对不同的目标类别使用了不同的补丁，但我们的目标是学习一种可以欺骗图像中所有类别的通用局部扰动。这将有助于了解使用不同的远程上下文连接在现代网络中攻击的传播，此类网络最容易受到普遍攻击，而它们对FCN的影响仅限于受干扰的区域。

<img src=".\img\Indirect local attack\Universal.png" alt="Universal" style="zoom:67%;" />

#### Adversarial Attack Detection

 为了了解上述攻击的强度，我们介绍了一种可以在**全局图像级别或像素级别**起作用的检测方法。

后者在间接攻击的情况下特别有趣，在这种情况下，**扰动区域和欺骗区域不同**。在这种情况下，**我们的目标是定位被欺骗的像素，这比查找被干扰的像素更具挑战性，因为它们的强度值没有改变**

<img src=".\img\Indirect local attack\detection.png" alt="detection" style="zoom:67%;" />
...
在本文中，我们研究了间接局部图像扰动对现代语义分割网络性能的影响。我们已经观察到，最**新的分割网络（例如PSANet和PSPNet）更容易受到局部干扰，因为它们对上下文的使用提高了其在清晰图像上的准确性，从而可以将干扰传播到较远的图像区域**。因此，它们可能会受到扰动的干扰，扰动仅覆盖图像区域的2.3％。然后，我们提出了一种**基于Mahalanobis距离的检测策略**，该策略已被证明对于图像级和像素级攻击检测都是有效的。尽管如此，仍可以改善以像素方式定位受骗区域的性能，这将是我们未来的目标。
...
[MI-FGSM]: file://E:/研究生学习/论文/研0/Adversarial/1710.06081MI-FGSM.pdf



### 期刊/会议

NIPS 2017 Adversarial Attacks and Defenses Competition中non-targeted attack和targeted attack赛道的**第一名**，投稿至**CVPR 2018**
...
我们提出了一类**基于动量的迭代算法**来增强对抗性攻击。通过**将动量项整合到攻击的迭代过程**中，我们的方法可以**稳定更新方向**，并**在迭代过程中摆脱不良的局部最大值**，从而产生**更具可传递性**的对抗样本。

为了进一步提高黑盒攻击的成功率，我们将动量迭代算法应用于一组模型，并证明具有强大防御能力的经过对抗训练的模型也容易受到我们的黑盒攻击的攻击。

### Introduction

可转移性的现象是由于不同的机器学习模型在数据点周围学习相似的决策边界这一事实所致，因此为一种模型设计的对抗样本也对其他模型有效。

**基于优化和迭代方法生成的对抗样本的可传递性较差，因此使黑盒攻击的有效性降低**

 在本文中，**我们提出了一系列基于动量迭代梯度的方法，以提高生成的对抗样本的成功率**。 除了基于迭代梯度的方法用梯度来迭代扰动输入以使损失函数最大化以外，**基于动量的方法还在迭代过程中沿损失函数的梯度方向累积速度矢量，为了稳定更新方向并避免出现不良的局部最大值**。我们表明，由动量迭代方法生成的对抗样本在白盒和黑盒攻击中均具有较高的成功率。所提出的方法减轻了白盒攻击和可传递性之间的折衷，并且比单步方法和原始迭代方法具有更强的攻击算法。

#### 贡献

1. 引入一类攻击算法，称为基于动量迭代梯度的方法，其中，我们**在每次迭代时都会累积损失函数的梯度，以稳定优化并避免不良的局部最大值**；
2. 研究了**几种同时攻击多个模型的整体方法，这些方法通过保持较高的攻击成功率证明了强大的可传递性**；
3. 是第一个证明通过具有强大防御能力的整体对抗训练获得的模型也容易受到黑盒攻击的攻击。



### Backgrounds

#### 介绍了现有的Attack Methods

##### One-step gradient-based approaches

FGSM，找到一个对抗样本`x*`，最大化loss 函数`J(x*,y)`，`J`通常为`cross-entropy loss`

<img src=".\img\MI-FGSM\FGSM.png" alt="FGSM" style="zoom:80%;" />
...
<img src=".\img\MI-FGSM\I-FGSM.png" alt="I-FGSM" style="zoom:80%;" />

##### Optimization-based methods

**基于优化的方法**会直接优化真实样本与对抗样本之间的距离，这取决于对抗示例的错误分类。常见的是盒约束的L-BFGS，主要是优化

<img src=".\img\MI-FGSM\L-BFGS.png" alt="L-BFGS" style="zoom:80%;" />
...
提出了一系列**基于动量迭代梯度的方法**来生成对抗样本，这些样本可以欺骗白盒模型和黑盒模型。

首先说明如何将动量集成到I-FGSM中，该方法引入了动量迭代快速梯度符号方法(MI-FGSM)来以非目标攻击方式生成满足L∞ 范数限制的对抗样本。然后，我们介绍了几种有关如何有效攻击模型集成的方法。最终，我们将MI-FGSM扩展到 L2 范式边界和有针对性的攻击，从而产生了一大类的攻击方法。

#### MI-FGSM算法

<img src=".\img\MI-FGSM\MI-FGSM算法图.png" alt="MI-FGSM算法图" style="zoom:80%;" />

动量法是一种通过在迭代过程中沿损失函数的梯度方向累积速度矢量来加速梯度下降算法的技术

<img src=".\img\MI-FGSM\MI-FGSM介绍.png" alt="MI-FGSM介绍" style="zoom:80%;" />

#### 攻击集成模型

集成方法已广泛用于研究和竞赛中，以提高性能和鲁棒性。集成的概念也可以应用于对抗性攻击，因为如果一个示例仍然对多个模型具有对抗性，那么它可能会捕获一个固有的方向，该方向总是使这些模型蒙蔽，并且更有可能同时转移到其他模型，从而启用强大的黑盒攻击。

作者提出一种ensemble in logits的攻击方法：攻击将logit激活融合在一起的多个模型。

强调了**使用logits融合模型的方法可以综合多个模型的细致输出，但也指出了这种方法可能会揭示出模型的脆弱性**

 具体来说，要攻击K个模型的集合，我们将对数融合为：

<img src=".\img\MI-FGSM\attack logit.png" alt="attack logit" style="zoom:67%;" />

#### 扩展

动量迭代方法可以轻松地推广到其他攻击设置。通过**用所有先前步骤的累积梯度替换当前梯度**，可以将任何迭代方法扩展到其动量变量。
...
在本文中，我们提出了多种基于动量的迭代方法来增强对抗性攻击，这些方法可以有效地欺骗白盒模型和黑盒模型。我们的方法始终以黑盒方式胜过基于梯度的一步法和基础的迭代法。我们进行了广泛的实验，以验证所提出方法的有效性，并解释为什么它们可以在实践中起作用。为了进一步提高生成的对抗样本的可传递性，我们建议攻击其logit融合在一起的模型集合。我们表明，通过整体对抗训练获得的模型容易受到我们的黑盒攻击，这为开发更强大的深度学习模型提出了新的安全问题。
...
[One Pixel]: file://E:/研究生学习/论文/研0/Adversarial/1710.08864One_Pixel.pdf
...
研究了只有一个pixel可以被修改的attack，基于differential evolution（DE）提出了one-pixel attack



### Introduction

**提出用差分进化(differential evolution, DE)提出一种单像素的黑盒攻击方法，唯一可获取的信息就是概率标签**。这个工作和之前的工作相比，主要优点是：

1. **有效性**。在Kaggle CIFAR-10数据集上，仅通过修改三个常见的深度神经网络结构上的一个像素就可以发起非目标攻击，成功率为68.71％，71.66％和63.53％。我们还发现，每个自然图像都可以被扰乱到1.8、2.1和1.5的其他类别。在攻击场景更为有限的原始CIFAR-10数据集上，我们显示出22.60％，35.20％和31.40％的成功率。在ImageNet数据集上，通过更改一个像素也可以对BVLC AlexNet模型进行非目标攻击，表明可以攻击16.04％的测试图像。
2. **半黑盒攻击**。仅需要黑盒反馈（**概率标签**），而无需目标DNN的内部信息，例如梯度和网络结构。我们的方法也比现有方法简单，因为它没有将搜索扰动的问题抽象为任何显式目标函数，而是**直接关注于增加目标类的概率标签值。**
...
<img src=".\img\One_Pixel\问题描述.png" alt="问题描述" style="zoom:80%;" />

<img src=".\img\One_Pixel\one pixel 可视化.png" alt="one pixel 可视化" style="zoom:67%;" />

- 绿点（球体）表示自然图像
- 在one-pixel 扰动的情况下，其搜索空间是在自然图像点相交的三条垂直线（红线和黑线）
- 在two-pixel 扰动的情况下，其搜索空间是三个蓝色的二维平面
...
差分进化（DE）是用于解决复杂的多模式优化问题的基于总体的优化算法。
...
1. **可以以更大的概率找到全局最优解。**DE不像梯度下降或者贪婪算法那样受局部影响较大（保持多样性的机制及子候选解集影响）。本文中有一个很强的限制条件，（只允许修改一个像素）使问题变得更加复杂。
2. **需要得信息更少。**DE不需要梯度下降、拟牛顿法等优化方法需要的信息进行优化求解。这对于对抗图片的生成是至关重要的。（1）有些网络是不可微的（2）为了求解梯度信息需要获得目标系统更多的信息，有时是无法获得的。
3. **简易性。**本文提出的方法与分类器无关，获得类别的概率就足够。



#### 方法和设置

 我们**将扰动编码为一个数组（候选解），该数组通过差分运算进行优化（演化）**。一**个候选解包含固定数量的扰动，并且每个扰动都是一个元组**，该元组包含五个元素：**x-y坐标和该扰动的RGB值。**一个扰动会修改一个像素。**候选解（子代）的初始数量为400，在每次迭代中，将使用常规的DE公式生成另外400个候选解（子代）**：

<img src=".\img\One_Pixel\DE公式.png" alt="DE公式" style="zoom:80%;" />
...
- Adversarial Probability Labels（**Confidence**）：累加每次成功扰动为目标类别的概率值，然后除以成功扰动的总数。表示模型对对抗图像产生“误分类”的confidence。
- Number of Target Classes：计算成功扰动到一定数量类别的图片的数量。尤其，**计算无法被其他类别扰动的图片数量，可以评估非目标攻击的有效性。**
- Number of Original-Target Class Pairs：计算原始目标类别对被攻击的次数
...
- **另一个分析**
 ...
 这个方法的局限性在于**是个搜索，而且图像尺寸越大效果越差**，但是如果迭代次数够，就能找到比较好的解。
...
[Boundary Attack]: file://E:/研究生学习/论文/研0/Adversarial/1712.04248Boudary_attack.pdf
...
在本文中作者将之前的对抗攻击分为三类：**gradient-based, score-based和transfer-based attacks**。

1. gradient-based：知道模型的细节信息（梯度、结构），利用对应图片的梯度进行攻击；
2. score-based：不知道模型的细节信息，但是**知道模型预测的概率**或者logits；
3. transfer-based：不依赖模型的信息，但是**依赖训练数据的信息**，可以**利用训练数据训练替代模型，从而生成可迁移的扰动**；如果**对抗样本是攻击很多替代模型集成而得到的，攻击成功率可以达到近100%**。

作者提出了一种只依赖于模型最终decision的attack方法，定义为**decision-based attack**，其有以下几个优点：

1. 更接近现实场景中的黑盒模型，比如无人驾驶；
2. 对攻击模型不需要很多的知识，在实现上迁移攻击容易；
3. 比基于梯度和score的攻击更鲁棒。

作者针对decision-based attack提出了Boudary Attack，它从一个很大的扰动开始，然后逐渐寻找减小扰动大小

<img src=".\img\Boundary Attack\decision attack.png" alt="decision attack" style="zoom:67%;" />

### Introduction

详细区分了当前的三种攻击方法

- Gradient-based attacks：知道模型的细节信息（梯度、结构），利用对应图片的梯度进行攻击；例如FGSM、BIM、DeepFool、CW
  - 防御方法：梯度mask、防御性蒸馏
- Score-based attacks：不知道模型的细节信息，但是**知道模型预测的概率**或者logits。例如JSMA的黑盒形式，CW的黑盒攻击形式
  - 防御方法：在模型种添加随机元素（例如Dropout）可以直接严重阻碍数值梯度估计；sharp-edged plateau
- Transfer-based attacks：不依赖于模型信息，但是依赖于训练数据。用训练数据训练替代模型，产生具有迁移性的对抗样本。如果**对抗样本是攻击很多替代模型集成而得到的，攻击成功率可以达到近100%**。
  - 防御方法：在训练数据集中做数据增强（**用对抗样本做数据增强，对抗样本是由其他替代模型产生的集合**）

本文的贡献

- 强调了decision-based attacks是一种重要的攻击类型，它更适合在现实场景中使用
...
换句话说，就是我们使用合适的**proposal distribution** `P` 进行**rejection sampling**，**根据给定的对抗标准 c(.) 寻找逐渐变小的对抗扰动**

<img src=".\img\Boundary Attack\算法流程.png" alt="算法流程" style="zoom:80%;" />

#### 初始化

Boundary attack需要最开始样本已经是对抗样本。在untargeted场景，根据最大熵分布对每个pixel从 u(0,255) 采样；在targeted场景下，直接选用target类别的图像开始。

#### Proposal Distribution

算法的效果依赖于proposal distribution `P`，即在每一次step中的随机探索的方向。

<img src=".\img\Boundary Attack\proposal distribution.png" alt="proposal distribution" style="zoom:80%;" />

- 扰动样本要在input domain中
- 扰动的相应大小为δ
- 扰动能减少perturbed image和original image之间的距离，大概值为ε

实际上操作时，先从N(0,1)开始采样，然后进行处理，使其满足上述条件

#### 超参数调整

 算法主要的超参数是扰动总长度 δ 和到原图距离的步长 ε 。这个图意思就是，所谓正交扰动，就是**先水平动一下，然后垂直靠近边界，动态调整参数**。

<img src=".\img\Boundary Attack\算法图.png" alt="算法图" style="zoom:67%;" />

- 左图：本质上，Boundary Attack就是沿着对抗性和非对抗性图像之间的边界执行rejection sample
- 中间的图：在每个步骤中，我们通过 (#1) 从独立同分布高斯绘制并投影到球体上，以及 (#2) 向目标图像稍微移动来绘制新的随机方向
- 右图：根据边界的局部几何形状动态调整（正交步长和原始输入步长）



### Experiments

在MNIST、CIFAR-10和ImageNet-1000上进行了实验
...
[Optimization Attack]: file://E:/研究生学习/论文/研0/Adversarial/1807.04457Optimization_Attack.pdf
...
JD AI Research, IBM Research

### Abstract

研究了在硬标签黑盒设置中攻击机器学习模型的问题，即除了攻击者可以进行查询来探测相应的硬标签决策之外，不会透露任何模型信息。

但是存在的挑战是：**需要最小化一个非连续的阶跃函数，这是一个组合问题，无法用基于梯度的优化器来解决**。

目前的解决方法有：Boundary Attack，缺点是需要大量查询且不一定保证收敛。
...
提出了一个optimization-based的framework

之前的工作失败原因有以下两点：

- 首先，梯度不能直接通过反向传播计算，并且基于有限差分的方法也会失败，因为**硬标签输出对小输入扰动不敏感**
- 因为只能拿到分类标签，所以攻击目标函数是不连续的，因此难以优化；

现有的攻击方法是Boundary Attack，但是它需要超多次的查询，而且无法保证一定能收敛。因此，作者将问题重新表述为**实数值优化问题**（连续而且可以使用零阶梯度估计），使用RGF（Randomized Gradient-Free method）可以收敛，可以取得超过Boundary Attack的性能，同时也是**第一个能保证收敛**的decision-based攻击。

<img src=".\img\Optimization Attack\hard-label.png" alt="hard-label" style="zoom:80%;" />

- 图（a）：一个分类器的决策边界
- 图（b）：L(Z(x))项是logits层的loss，它是连续的（适用于score-based的场景）
- 图（c）：L(f(x))项是output-label的loss，它是离散的，因此目标函数不连续
- 图（d）：作者提出的攻击目标函数
...
<img src=".\img\Optimization Attack\目标函数.png" alt="目标函数" style="zoom:80%;" />

- θ是搜索方向
- g(θ)是沿着方向θ从x0到最接近的对抗样本的距离

- 不同于搜索对抗样本，作者做的是搜索方向θ从而最小化g(θ)
- 最终，对抗样本被通过以下式子找到<img src=".\img\Optimization Attack\找到对抗样本.png" alt="找到对抗样本" style="zoom:80%;" />

**g(θ) 将输入方向映射到实值输出（到决策边界的距离），通常是连续的**，如下图所示

<img src=".\img\Optimization Attack\解释g.png" alt="解释g" style="zoom:80%;" />

#### 如何计算g(θ)

<img src=".\img\Optimization Attack\如何计算g.png" alt="如何计算g" style="zoom:80%;" />
...
<img src=".\img\Optimization Attack\RGF.png" alt="RGF" style="zoom:80%;" />

实际上做的时候是sample `q`个向量，然后将他们的平均估计作为梯度，q=20
...
提出了optimization-based的hard-label black-box的攻击，适用于离散和非连续的模型。它的query次数更少，理论上有收敛保障，和最好的方法相比可以取得更小的失真，query次数减少3、4倍。
